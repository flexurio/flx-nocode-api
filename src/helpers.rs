...
fn is_safe_mime_type(mime: &str) -> bool {
    ...
    allowed.iter().any(|p| if p.ends_with('/') { mime.starts_with(p) } else { mime == *p })
}
...
pub async fn multipart_to_json(mut multipart: Multipart) -> Result<Value, actix_web::Error> {
    ...
    let max_field_kb: usize = std::env::var("UPLOAD_TEXT_LIMIT_KB").ok().and_then(|s| s.parse().ok()).unwrap_or(1024);
    let max_field_size: usize = max_field_kb * 1024;
    let max_files: usize = std::env::var("UPLOAD_MAX_FILES").ok().and_then(|s| s.parse().ok()).unwrap_or(10);
    let max_fields: usize = std::env::var("UPLOAD_MAX_FIELDS").ok().and_then(|s| s.parse().ok()).unwrap_or(200);
    ...
    while let Some(item) = multipart.next().await {
        ...
        if let Some(filename) = content_disposition.as_ref().and_then(|cd| cd.get_filename()) {
            file_count += 1;
            ...
            if let Some(ref allow) = allowed_ext {
                if let Some(ext) = std::path::Path::new(&safe_name).extension().and_then(|e| e.to_str()).map(|s| s.to_lowercase()) {
                    if !allow.contains(&ext) {
                        return Err(actix_web::error::ErrorBadRequest("File extension not allowed"));
                    }
                }
            }
            ...
            if image_storage == "DB" {
                ...
                while let Some(chunk) = field.next().await {
                    let data = chunk?;
                    total_size += data.len();
                    if total_size > max_file_size { return Err(actix_web::error::ErrorPayloadTooLarge("File too large")); }
                    if sniff_buf.len() < 8192 { let take = 8192 - sniff_buf.len(); sniff_buf.extend_from_slice(&data[..data.len().min(take)]); }
                    buffer.extend_from_slice(&data);
                }
                if let Some(kind) = infer::get(&sniff_buf) {
                    let detected = kind.mime_type();
                    if !is_safe_mime_type(detected) { return Err(actix_web::error::ErrorBadRequest("Detected MIME not allowed")); }
                    // if allowed_ext is set, ensure detected extension is allowed
                    if let Some(ref allow) = allowed_ext {
                        let ext = kind.extension();
                        if !allow.contains(&ext.to_lowercase()) { return Err(actix_web::error::ErrorBadRequest("Detected extension not allowed")); }
                    }
                }
                ...
                let mut f = match std::fs::OpenOptions::new().create(true).write(true).truncate(true).open(&file_path) {
                    Ok(f) => f,
                    Err(e) => return Err(actix_web::error::ErrorInternalServerError(format!("Write open failed: {}", e))),
                };
                let mut total_size = 0usize;
                let mut sniff_buf: Vec<u8> = Vec::with_capacity(8192);
                ...
                while let Some(chunk) = field.next().await {
                    ...
                    if !detected_checked {
                        if sniff_buf.len() < 8192 { let take = 8192 - sniff_buf.len(); sniff_buf.extend_from_slice(&data[..data.len().min(take)]); }
                        if sniff_buf.len() >= 32 || data.is_empty() { // have enough to detect
                            if let Some(kind) = infer::get(&sniff_buf) {
                                let detected = kind.mime_type();
                                if !is_safe_mime_type(detected) {
                                    let _ = std::fs::remove_file(&file_path);
                                    return Err(actix_web::error::ErrorBadRequest("Detected MIME not allowed"));
                                }
                                if let Some(ref allow) = allowed_ext {
                                    let ext = kind.extension();
                                    if !allow.contains(&ext.to_lowercase()) { let _ = std::fs::remove_file(&file_path); return Err(actix_web::error::ErrorBadRequest("Detected extension not allowed")); }
                                }
                            }
                    ...
                    if let Err(e) = f.write_all(&data) {
                        return Err(actix_web::error::ErrorInternalServerError(format!("Write failed: {}", e)));
                    }
                }
            ...
            while let Some(chunk) = field.next().await {
                let data = chunk?;
                total_size += data.len();
                if total_size > max_field_size { return Err(actix_web::error::ErrorPayloadTooLarge("Field too large")); }
