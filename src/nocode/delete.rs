...
pub async fn delete(
    ...
    match transaction.query_with_params(&s_sql, bind_params.clone()).await {
        Ok(_) => {
            let (is_fk_ok, err_message) = {
                let type_process: &str = "DELETE";
                let state = state.clone();
                let transaction: &mut Box<dyn DbTransaction> = &mut transaction;
                let id_user = claims.id;
                let id_data = id_raw.clone();
                let id_new = "".to_string();
                async move {
