              ...
              if type_process == "DELETE" {
                     ...
                     if data_table.action == "cascade" {
                            if data_table.type_delete == "soft" {
                                   ...
                                   s_sql_fk = format!("DELETE FROM {} WHERE {} = ?", data_table.table, data_table.column);
                            } else {
                            ...
                            s_sql_fk = format!(
                                   ...
                                   data_table.table, data_table.column, state.query_converter.datetime_now, data_table.column
                            );
                                          ...
                                          if !rows.is_empty() {
                                                 if let Some(count) = rows[0].get(0) {
                                                        if count.as_i64().unwrap_or(0) > 0 {
                                                               ...
                                                               error_message = format!("Cannot delete data because it is referenced in table {}", data_table.table);
...
async fn execute(transaction: &mut Box<dyn DbTransaction>, s_sql_fk: &str, bind_params_fk: Vec<DbParam>, type_process: &str) -> Result<Vec<Value>, anyhow::Error> {
       log_output(
